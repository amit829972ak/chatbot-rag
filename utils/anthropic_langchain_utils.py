...
from utils.anthropic_utils import DEFAULT_MODEL, get_anthropic_client, ANTHROPIC_MODELS

def create_rag_chain(api_key=None, model_version=None):
    """
    Create a RAG chain using LangChain with Anthropic Claude.
    ...
    For structured data like CSV, TSV, XLSX content, format your response in a clear, tabular way when appropriate.
    For PDF content, include page references when relevant.
...
def get_rag_response(query, context, api_key=None, model_version=None):
    """
    Get a RAG-enhanced response using the provided context with Anthropic.
    ...
    try:
        ...
        return f"Error getting RAG response from Anthropic: {str(e)}"

def get_multimodal_response(query, image_analysis, api_key=None, model_version=None):
    """
    Get a response that incorporates both text query and image analysis using Anthropic.
    ...
    try:
        ...
        return f"Error getting multimodal response from Anthropic: {str(e)}"
