...
if "visited" not in st.session_state:
    ...
    with welcome_container:
        ...
        with st.expander("How to use this chatbot", expanded=True):
            st.markdown("""
            This chatbot allows you to interact with AI models through text, images, and documents. Here's how to use it:
...
with st.sidebar:
    ...
    selected_model = st.radio(
        ...
        format_func=lambda x: "Google Gemini" if x == "gemini" else ("OpenAI GPT" if x == "openai" else "Anthropic Claude"),
        index=0 if st.session_state.selected_model == "gemini" else (1 if st.session_state.selected_model == "openai" else 2),
    ...
    if selected_model == "gemini":
        ...
        with st.expander("How to get a Google Gemini API key"):
            ...
            **Note:** Google Gemini API has a free tier with Gemini 1.0 Pro. Check Google's documentation for more details.
            """)
    elif selected_model == "openai":  # OpenAI model selected
        ...
        with st.expander("How to get an OpenAI API key"):
            ...
            **Note:** OpenAI offers a free tier with $5 credit for new users. GPT-3.5 Turbo is the most affordable model.
            """)
    else:  # Anthropic model selected
        st.subheader("🔑 Anthropic API Key")
        
        # Import model options from anthropic_utils.py
        from utils.anthropic_utils import ANTHROPIC_MODELS, DEFAULT_MODEL
        
        # Model version selection dropdown for Anthropic
        st.subheader("🤖 Select Anthropic Model Version")
        
        # Initialize model version in session state if not already there
        if "anthropic_model_version" not in st.session_state:
            st.session_state.anthropic_model_version = DEFAULT_MODEL
        ...
        for model_id, model_info in ANTHROPIC_MODELS.items():
            display_name = f"{model_info['name']}"
            anthropic_model_options.append(model_id)
            model_descriptions[model_id] = model_info['description']
        
        # Model selection dropdown
        selected_anthropic_model = st.selectbox(
            "Choose Anthropic model version:",
            options=anthropic_model_options,
            format_func=lambda x: f"{ANTHROPIC_MODELS[x]['name']} - {ANTHROPIC_MODELS[x]['description']}",
            index=anthropic_model_options.index(st.session_state.anthropic_model_version) if st.session_state.anthropic_model_version in anthropic_model_options else 0,
            help="Select which Anthropic model version to use. Claude 3.5 Sonnet is the latest and most capable model."
        )
        
        # Check if model version has changed
        if selected_anthropic_model != st.session_state.anthropic_model_version:
            # Update the session state with the selection
            st.session_state.anthropic_model_version = selected_anthropic_model
            
            # Reinitialize vector store with the new model version if API key is available
            if st.session_state.get("anthropic_api_key"):
                st.session_state.vector_store = initialize_vector_store(selected_anthropic_model)
        
        # Initialize Anthropic API key in session state if not already there
        ...
        if api_key:
            if len(api_key) < 20:
                st.warning("⚠️ Anthropic API key appears to be too short. Keys are typically longer.")
                st.session_state.anthropic_api_key = api_key  # Keep what the user entered
            else:
                st.success("✅ Anthropic API key format looks valid!")
                st.session_state.anthropic_api_key = api_key  # Keep what the user entered
        else:
            st.warning("⚠️ Please enter your Anthropic API key to use this model.")
        ...
        with st.expander("How to get an Anthropic API key"):
            ...
            **Note:** Anthropic's Claude 3.5 Sonnet is their latest model with excellent document handling and structured data capabilities.
            """)
...
elif st.session_state.selected_model == "anthropic":
    st.session_state.agent.set_model(
        "anthropic", 
        st.session_state.anthropic_api_key, 
        st.session_state.anthropic_model_version if "anthropic_model_version" in st.session_state else None
    )
else:  # OpenAI
    st.session_state.agent.set_model(
        "openai", 
